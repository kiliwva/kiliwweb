...
export function afterAuthRedirect(request) {
  ...
  if (isPlainHost(host)) return '/dash';
  /* signing in on the account hub / Minecraft host keeps you there */
  if (host.startsWith('id.') || host.startsWith('mcid.')) return `${proto}//${host}/`;
  return `${proto}//cloud.${baseHost(host)}/dash`;
}
...
export function authRedirect(request) {
  const host = hostname(request);
  const proto = new URL(request.url).protocol;
  if (isPlainHost(host)) return '/login';
...
/* ---------- Telegram panel sessions (mcid.<domain>) ----------
   A login separate from the account session above: identifies a
   Telegram user, not an email, and is host-only to the panel domain
   so it never doubles as a cloud account login. */

const MCID_TTL = 60 * 60 * 24 * 30; // 30 days

export function buildMcidCookie(request, token, maxAge = MCID_TTL) {
  const url = new URL(request.url);
  let cookie = `kiliw_mcid=${token}; Path=/; Max-Age=${maxAge}; HttpOnly; SameSite=Lax`;
  if (url.protocol === 'https:') cookie += '; Secure';
  return cookie; // no Domain → scoped to the panel host only
}

export async function createMcidSession(env, tgUser, request) {
  const token = randomHex(32);
  await env.KILIW_FILES.put(`_auth/mcidsess/${token}.json`, JSON.stringify({
    tgId: tgUser.tgId,
    tgUsername: tgUser.tgUsername || '',
    created: Date.now(),
    expires: Date.now() + MCID_TTL * 1000,
  }));
  return { token, cookie: buildMcidCookie(request, token) };
}

export async function getMcidSession(request, env) {
  if (!storageReady(env)) return null;
  const token = getCookie(request, 'kiliw_mcid');
  if (!token || !/^[0-9a-f]{64}$/.test(token)) return null;
  const obj = await env.KILIW_FILES.get(`_auth/mcidsess/${token}.json`);
  if (!obj) return null;
  try {
    const s = JSON.parse(await obj.text());
    if (!s || !s.tgId) return null;
    if (s.expires && s.expires < Date.now()) {
      await env.KILIW_FILES.delete(`_auth/mcidsess/${token}.json`);
      return null;
    }
    return { ...s, token };
  } catch {
...
export async function destroyMcidSession(request, env) {
  const token = getCookie(request, 'kiliw_mcid');
  if (token && /^[0-9a-f]{64}$/.test(token)) {
    await env.KILIW_FILES.delete(`_auth/mcidsess/${token}.json`).catch(() => {});
  }
  return buildMcidCookie(request, 'deleted', 0);
}

/** Telegram IDs allowed to use the moderation side of the panel. */
export function isMcidAdmin(env, tgId) {
  const raw = String(env?.MCID_ADMINS || '');
  if (!raw || !tgId) return false;
  return raw.split(/[,\s]+/).filter(Boolean).includes(String(tgId));
}

/** Minecraft nicknames whose linked Telegram is treated as an admin. */
export function isMcidAdminNick(env, nick) {
  const raw = String(env?.MCID_ADMIN_NICKS || '');
  if (!raw || !nick) return false;
  const want = String(nick).toLowerCase();
  return raw.split(/[,\s]+/).filter(Boolean).map((n) => n.toLowerCase()).includes(want);
}

/** Kiliw ID emails that may open the Minecraft moderation panel. */
export function isMcidAdminEmail(env, email) {
  if (!email) return false;
  if (isOwner(env, email)) return true;
  const raw = String(env?.MCID_ADMIN_EMAILS || '');
  if (!raw) return false;
  const want = String(email).toLowerCase();
  return raw.split(/[,\s]+/).filter(Boolean).map((e) => e.toLowerCase()).includes(want);
}

/* ---------- stand-alone Minecraft accounts (email + password) ----------
   A separate account system for signing in to Minecraft on mcid.<domain>,
   completely independent of the main K-ID accounts. */

const MCACC_TTL = 60 * 60 * 24 * 30; // 30 days
const mcAccKey = (email) => `_auth/mcusers/${email}.json`;

export async function getMcAccUser(env, email) {
  const e = String(email || '').trim().toLowerCase();
  if (!e) return null;
  const obj = await env.KILIW_FILES.get(mcAccKey(e));
  return obj ? await obj.json().catch(() => null) : null;
}

export async function putMcAccUser(env, user) {
  await env.KILIW_FILES.put(mcAccKey(user.email), JSON.stringify(user));
}

export function buildMcAccCookie(request, token, maxAge = MCACC_TTL) {
  const url = new URL(request.url);
  let cookie = `kiliw_mcacc=${token}; Path=/; Max-Age=${maxAge}; HttpOnly; SameSite=Lax`;
  if (url.protocol === 'https:') cookie += '; Secure';
  return cookie; // host-only (scoped to the mcid host)
}

export async function createMcAccSession(env, email, request) {
  const token = randomHex(32);
  await env.KILIW_FILES.put(`_auth/mcaccsess/${token}.json`, JSON.stringify({
    email, created: Date.now(), expires: Date.now() + MCACC_TTL * 1000,
  }));
  return { token, cookie: buildMcAccCookie(request, token) };
}

export async function getMcAccSession(request, env) {
  if (!storageReady(env)) return null;
  const token = getCookie(request, 'kiliw_mcacc');
  if (!token || !/^[0-9a-f]{64}$/.test(token)) return null;
  const obj = await env.KILIW_FILES.get(`_auth/mcaccsess/${token}.json`);
  if (!obj) return null;
  try {
    const s = JSON.parse(await obj.text());
    if (!s || !s.email) return null;
    if (s.expires && s.expires < Date.now()) {
      await env.KILIW_FILES.delete(`_auth/mcaccsess/${token}.json`);
      return null;
    }
    return { ...s, token };
  } catch {
...
export async function destroyMcAccSession(request, env) {
  const token = getCookie(request, 'kiliw_mcacc');
  if (token && /^[0-9a-f]{64}$/.test(token)) {
    await env.KILIW_FILES.delete(`_auth/mcaccsess/${token}.json`).catch(() => {});
  }
  return buildMcAccCookie(request, 'deleted', 0);
}
...
/** Daily USD price of a plan (legacy Pro records default to the lowest tier). */
export function planDailyPrice(plan, gb) {
  const base = plan === 'dev' ? DEV_PRICE : (PRO_TIERS[gb] ?? PRO_TIERS[PRO_GB_MIN]);
  return base / PRO_DAYS;
}

/** Switching plans mid-term: the unused value of the current plan
    converts into extra days on the new one. Null when not an upgrade
    (no active plan, or buying the same tier = plain renewal). */
export function upgradePreview(user, plan, gb) {
  if (!(proActive(user) || devActive(user))) return null;
  const newPlan = plan === 'dev' ? 'dev' : 'pro';
  const newGb = newPlan === 'dev' ? DEV_GB : gb;
  if (user.plan === newPlan && user.planGb === newGb) return null;

  const day = 24 * 60 * 60 * 1000;
  const remainingDays = (user.planUntil - Date.now()) / day;
  const oldDaily = planDailyPrice(user.plan, user.planGb);
  const newDaily = planDailyPrice(newPlan, newGb);
  const extraDays = newDaily > 0 ? (remainingDays * oldDaily) / newDaily : 0;
  return {
    fromPlan: user.plan,
    fromGb: user.planGb,
    remainingDays: Math.round(remainingDays * 10) / 10,
    extraDays: Math.round(extraDays * 10) / 10,
  };
}
...
export async function applyProPurchase(env, email, gb, paymentId, plan = 'pro') {
  ...
  const newPlan = isDev ? 'dev' : 'pro';
  const newGb = isDev
    ? DEV_GB
    : Math.min(Math.max(Math.round(gb), PRO_GB_MIN), PRO_GB_MAX);

  const wasActive = proActive(user) || devActive(user);
  let until;
  if (wasActive && user.plan === newPlan && user.planGb === newGb) {
    /* renewal of the same tier stacks on top of the current period */
    until = user.planUntil + PRO_DAYS * day;
  } else if (wasActive) {
    /* plan switch: unused value of the old plan becomes extra days */
    const upgrade = upgradePreview(user, plan, newGb);
    until = now + (PRO_DAYS + (upgrade?.extraDays || 0)) * day;
  } else {
    until = now + PRO_DAYS * day;
  }

  user.plan = newPlan;
  user.planGb = newGb;
  user.planUntil = Math.round(until);
  ...
  await notifyAccountEvent(env, email, 'plan-activated', { plan: newPlan, gb: newGb });
  return true;
}
...
export async function heleketRequest(env, path, body) {
  ...
  try {
    const res = await fetch(`https://api.heleket.com/v1${path}`, {
      ...
      headers: {
        'Content-Type': 'application/json',
        /* Heleket's edge firewall rejects header-less server requests
           with "403 You are restricted to access the site" — look like
           a well-behaved API client */
        'User-Agent': 'Mozilla/5.0 (compatible; KiliwCloud/1.0; +https://kiliw.com)',
    ...
    const data = await res.json().catch(() => null);
    if (!res.ok) console.log(`heleket ${path} → ${res.status}: ${JSON.stringify(data).slice(0, 500)}`);
    return { ok: res.ok, status: res.status, data };
  } catch (err) {
    /* network failure must not take the whole request down with a 500 */
    console.log(`heleket ${path} fetch failed: ${err?.message}`);
    return { ok: false, status: 0, data: null };
  }
}

/** Short human-readable reason from a failed Heleket reply (for the UI). */
export function heleketErrorDetail(status, data) {
  if (!status) return 'network';
  const msg = data?.message
    || (data?.errors ? JSON.stringify(data.errors) : null)
    || data?.error
    || null;
  return msg ? `${status}: ${String(msg).slice(0, 200)}` : `HTTP ${status}`;
}
...
export function buildCodeEmail({ subject, intro, note, code }) {
  ...
  /* The Kiliw mosaic-K mark, built from coloured table cells so it renders
     in every email client (SVG and gradients are stripped by most). White
     stem in the left column, coral rays offset to the right. */
  const W = '#F2F2F2';
  const C = '#D97757';
  const sq = (color) => `<td style="width:9px;height:9px;background:${color};border-radius:3px;font-size:0;line-height:0;">&nbsp;</td>`;
  const gap = '<td style="width:9px;height:9px;font-size:0;line-height:0;">&nbsp;</td>';
  const markRows = [[W, null, C], [W, C, null], [W, C, null], [W, null, C]]
    .map((r) => `<tr>${r.map((c) => (c ? sq(c) : gap)).join('')}</tr>`).join('');
  const logo = `<table role="presentation" cellpadding="0" cellspacing="0"><tr>
        <td style="background:#141414;border:1px solid #2F2F2F;border-radius:12px;padding:8px;vertical-align:middle;">
          <table role="presentation" cellpadding="0" cellspacing="0" style="border-collapse:separate;border-spacing:3px;">${markRows}</table>
        </td>
        <td style="vertical-align:middle;padding-left:12px;font-family:${font};font-size:21px;font-weight:800;color:#F2F2F2;letter-spacing:-0.3px;">Kiliw</td>
      </tr></table>`;
...
/* ---------- captcha tickets ----------
   A user who already passed Turnstile + password check but still owes a
   2FA code gets a short-lived ticket, so entering the code does not
   demand solving the captcha again (each token is single-use). */

const CAPTCHA_TICKET_TTL = 10 * 60 * 1000;

export async function createCaptchaTicket(env, email) {
  const ctx = randomHex(32);
  await env.KILIW_FILES.put(
    `_auth/captcha/${ctx}.json`,
    JSON.stringify({ email, expires: Date.now() + CAPTCHA_TICKET_TTL }),
  );
  return ctx;
}

/** Valid, unexpired ticket for this email? (Non-consuming: wrong 2FA
    codes may be retried without re-solving the captcha.) */
export async function checkCaptchaTicket(env, ctx, email) {
  if (!ctx || !/^[0-9a-f]{64}$/.test(String(ctx))) return false;
  const obj = await env.KILIW_FILES.get(`_auth/captcha/${ctx}.json`);
  if (!obj) return false;
  const data = await obj.json().catch(() => null);
  if (!data || data.email !== email || data.expires < Date.now()) {
    await env.KILIW_FILES.delete(`_auth/captcha/${ctx}.json`).catch(() => {});
    return false;
  }
  return true;
}

export async function deleteCaptchaTicket(env, ctx) {
  if (!ctx || !/^[0-9a-f]{64}$/.test(String(ctx))) return;
  await env.KILIW_FILES.delete(`_auth/captcha/${ctx}.json`).catch(() => {});
}

/* ---------- password reset links ----------
   Admin-issued one-shot links: open the reset page with ?k=<token> and
   set a new password directly, no email code. 15-minute lifetime. */

const RESET_LINK_TTL = 15 * 60 * 1000;

export async function createResetLink(env, email) {
  const k = randomHex(32);
  await env.KILIW_FILES.put(
    `_auth/resetlinks/${k}.json`,
    JSON.stringify({ email, expires: Date.now() + RESET_LINK_TTL }),
  );
  return k;
}

/** Consume the link (single use); returns the email or null. */
export async function takeResetLink(env, k) {
  if (!k || !/^[0-9a-f]{64}$/.test(String(k))) return null;
  const key = `_auth/resetlinks/${k}.json`;
  const obj = await env.KILIW_FILES.get(key);
  if (!obj) return null;
  const data = await obj.json().catch(() => null);
  await env.KILIW_FILES.delete(key).catch(() => {});
...
/* ---------- HTTP Range parsing (video thumbnails / seeking) ---------- */

/** Parse a "Range: bytes=…" header against an object size.
    → null (no/unsupported header), {invalid:true}, or {offset,length,end}. */
export function parseRange(request, size) {
  ...
  if (Number.isNaN(start) || start > end || start >= size) return { invalid: true };
  return { offset: start, length: end - start + 1, end };
}

/* ---------- direct links (hotlink) ---------- */

/** Raw-bytes URL for a public, unprotected file share. */
export function directUrl(request, token) {
  const url = new URL(request.url);
  const host = isPlainHost(url.hostname) ? url.host : `cloud.${baseHost(url.hostname)}`;
  return `${url.protocol}//${host}/f/${token}`;
}

/* ---------- webhooks (DEV plan) ---------- */

const hookKey = (email) => `_hooks/${email}.json`;

export async function getWebhook(env, email) {
  const rec = await env.KILIW_FILES.get(hookKey(email));
  if (!rec) return null;
  try {
    return JSON.parse(await rec.text());
  } catch {
...
export async function saveWebhook(env, email, hook) {
  await env.KILIW_FILES.put(hookKey(email), JSON.stringify(hook), {
    httpMetadata: { contentType: 'application/json' },
  });
}

export async function deleteWebhook(env, email) {
  await env.KILIW_FILES.delete(hookKey(email));
}
...
/** Fire-and-forget delivery: one POST, HMAC-signed, 5 s timeout, no retry. */
export function fireWebhook(env, waitUntil, email, event, data) {
  if (!waitUntil) return;
  waitUntil((async () => {
    try {
      const hook = await getWebhook(env, email);
      if (!hook?.url) return;
      if (Array.isArray(hook.events) && hook.events.length && !hook.events.includes(event)) return;
      const body = JSON.stringify({ event, ...data, ts: Date.now() });
      await fetch(hook.url, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'User-Agent': 'kiliw-webhook/1',
          'X-Kiliw-Event': event,
          'X-Kiliw-Signature': await hmacHex(hook.secret, body),
        },
        body,
        signal: AbortSignal.timeout(5000),
      }).catch(() => {});
    } catch { /* never break the request over a webhook */ }
  })());
}

/* ---------- presigned uploads (DEV plan) ---------- */

const UPLOAD_TTL_DEFAULT = 60 * 60 * 1000; // 1 hour
const UPLOAD_TTL_MAX = 24 * 60 * 60 * 1000;

const uploadKey = (token) => `_upload/${token}.json`;

/** Single-use ticket: anyone holding the URL may PUT one file. */
export async function createUploadTicket(env, email, path, maxBytes, ttlMs) {
  const token = randomHex(16);
  const expires = Date.now() + Math.min(Math.max(ttlMs || UPLOAD_TTL_DEFAULT, 60 * 1000), UPLOAD_TTL_MAX);
  const ticket = { token, email, path, maxBytes: maxBytes || 0, expires, created: Date.now() };
  await env.KILIW_FILES.put(uploadKey(token), JSON.stringify(ticket), {
    httpMetadata: { contentType: 'application/json' },
  });
  return ticket;
}

export async function getUploadTicket(env, token) {
  if (!/^[0-9a-f]{32}$/.test(String(token))) return null;
  const rec = await env.KILIW_FILES.get(uploadKey(token));
  if (!rec) return null;
  try {
    const ticket = JSON.parse(await rec.text());
    if (ticket.expires < Date.now()) {
      await env.KILIW_FILES.delete(uploadKey(token));
      return null;
    }
    return ticket;
  } catch {
...
export async function consumeUploadTicket(env, token) {
  await env.KILIW_FILES.delete(uploadKey(token));
}

/* ---------- account event notifications ---------- */

/** Security-relevant changes land in the bell as well (type 'event'). */
export async function notifyAccountEvent(env, email, event, extra = {}) {
  try {
    await addNotification(env, email, { type: 'event', event, ...extra });
  } catch { /* the event itself must never fail over a notification */ }
}
